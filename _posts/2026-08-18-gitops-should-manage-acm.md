...
policies, placements, and bindings. RHACM then enforces the same intent on
...
| GitOps feeds RHACM              | `Policy`, `Placement`, and bindings onto the hub | RHACM governance      | A fleet of similar clusters, Platform Plus, compliance evidence |
...
RHACM policies are a *delivery mechanism* for the cluster-repo column across
...
deploy *applications* to those spokes. That is RHACM helping GitOps reach
workloads. It is the opposite direction of GitOps managing RHACM so RHACM can
...
this Application sync,” not “is this estate compliant.” RHACM already has the
...
syncs generated policy to RHACM. RHACM places it. The spoke never becomes the
...
is how the model dies in review. RHACM’s
...
RHACM documentation has you copy the plug-in from the RHACM application
...
that generates policy is not an unmodified OpenShift GitOps instance. Install and image-pin RHACM
...
A generator file is the control surface. This shape matches the RHACM 2.16
...
and ClusterSets: this spoke gets virtualization, that spoke gets RHACS, the
...
poor substitute for RHACM labels when the question is membership in a
capability set.
...
manages RHACM; RHACM policies—mostly PolicyGenerator directories—manage the
...
4.20.x and RHACM 2.17; this site’s product line is OpenShift 4.22 and RHACM
...
- GitOps on the hub owns RHACM; RHACM owns spokes.
...
vocabulary is convenient. It is not an RHACM API. If you fork, you own the
fork.
...
- You already operate a kustomize factory with namespace folders and
  syncwaves, and RHACM would only wrap the same unique trees.
...
4. **GitOpsCluster is the other direction** — it registers spokes so Argo
   can ship apps. Do not use it as a synonym for GitOps managing RHACM.
