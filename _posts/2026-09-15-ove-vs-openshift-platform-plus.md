...
That is the box. These are not:

| Product | In the OVE SKU? | How it actually shows up |
| ------- | --------------- | ------------------------ |
| [Red Hat Advanced Cluster Management for Kubernetes (RHACM)](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_management_for_kubernetes/2.16/) | No | [ACM for Virtualization](/posts/acm-openshift-virtualization/) is an add-on |
| Ansible Automation Platform | No | Virtualization add-on; managed-node subscriptions are separate |
| Software-defined storage / OpenShift Data Foundation (ODF) | No | Buy ODF, a partner SDS, or array CSI. ODF Essentials is in Platform Plus |
| [Red Hat Advanced Cluster Security for Kubernetes (RHACS)](https://docs.redhat.com/en/documentation/red_hat_advanced_cluster_security_for_kubernetes/) | No | Platform Plus, or a standalone add-on |
| Quay as an estate registry | No | Bootstrap mirror only; estate Quay is Plus |

OVE *does* let those products run as infrastructure containers on the
cluster—CSI drivers, backup agents, an RHACM hub, Ansible Automation
Platform, SDS that exists to serve VM disks. Allowed to run is not
included in the SKU. Confirm borderline workloads with Red Hat. Fleet
operations for VM-only clusters still mean buying ACM for Virtualization,
not assuming RHACM arrived with OVE.
...
- The hub is a separate buy: ACM for Virtualization. Mixed ROSA and OCP
  spokes are not in scope for this fleet.
...
1. **OVE is the VM-only OpenShift SKU** — unlimited VMs and GitOps for
   virt. Not guest application containers. Not RHACM, Ansible Automation
   Platform, SDS, ODF, RHACS, or Quay as an estate registry. Those may
   run as infrastructure containers; you still buy the product.
